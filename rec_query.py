...
summary_sdf = (
    final_call_level_sdf
    .groupBy("center_location")
    .agg(
        F.count("*").alias("eligible_calls"),
        F.sum("adhered_call").alias("adhered_calls"),
        F.sum("slide_call").alias("slide_calls"),
        F.sum("all_plans_call").alias("all_plans_calls"),
        F.avg("adhered_call").alias("adherence_rate"),
        F.avg("slide_call").alias("slide_rate"),
        F.avg("all_plans_call").alias("all_plans_rate"),
    )
)

summary_sdf.createOrReplaceTempView("CALL_LEVEL_PITCHES_AND_RECS_SUMMARY")

print(f"Using pitch ordering column: {pitch_order_col}")
print(f"Filtering center_location to: {TARGET_CENTER_LOCATIONS}")
print("Exclusions are now flagged rather than filtered: in_arcadia_target, failed_qualification, has_payless_pitch, has_low_rec")
print("happy_path = 1 for calls passing all exclusions; 0 otherwise")
print("GCV sourced from gcv_v2 directly (no PAR multiplication)")
print("Temp view 'CALL_LEVEL_PITCHES_AND_RECS' created; row count:", final_call_level_sdf.count())
print("Temp view 'CALL_LEVEL_PITCHES_AND_RECS_SUMMARY' created")

final_call_level_sdf.toPandas().to_csv("call_level_pitches_and_recs.csv", index=False)
print("Wrote", final_call_level_sdf.count(), "rows to call_level_pitches_and_recs.csv")